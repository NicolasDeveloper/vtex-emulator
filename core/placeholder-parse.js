...
module.exports.parse = async (controller, templateString) => {
    ...
    const tags = templateString && templateString.match(/\<vtex\:contentPlaceHolder id\=\".*\".\/\>/g);
    ...
    if (tags) {
        ...
        for (let index = 0; index < searcheds.length; index++) {
            ...
            if (placeholder && placeholder.contents && placeholder.contents.length > 0) {
                ...
                templateString = templateString.replace(placeholder.tag, `placeholder não encontrado: ${placeholder.tag.match(/id\=\".*\"/g)[0]} `)
            }
        }
